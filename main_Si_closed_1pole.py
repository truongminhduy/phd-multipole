...
# # ######## SPECTRAL PROBLEM ########################
n_eig = np.array([120,10])
n_region = len(n_eig)
n_cum    = np.cumsum([0, *n_eig])
eigs     = np.zeros(sum(n_eig),dtype=complex)
dd       = np.zeros(sum(n_eig),dtype=complex)
# ##0
print_w2_chi_new(num2,den,1.4,30,n_eig[0])
slepc_options_rational = ' -nep_max_it 100 -nep_type nleigs -nep_rational -nep_target_magnitude -rg_interval_endpoints -50,50,-64.6,64.6'
...
####1
print_w2_chi_new(num2,den,1,80,n_eig[1])
slepc_options_rational = ' -nep_max_it 100 -nep_type nleigs -nep_rational -nep_target_magnitude -rg_interval_endpoints -60,60,64.5,100'
eigen_getdp = str_getdp_path+'getdp spectral_closed.pro -pre Projection -msh '+mesh_filename+' -cal -pos postop_e -slepc'# -v 0' 
os.system(eigen_getdp+ slepc_options_rational)
...
for x in range(n_region):
	...
	# os.system('rm norm1.txt norm2.txt norm3.txt')
	# norm_getdp = str_getdp_path+'getdp norm_closed.pro -pre Projection -msh '+mesh_filename+' -res spectral'+str(x)+'.res -pos postop_norm -v 0'
	# os.system(norm_getdp)
	# norm1 = np.loadtxt('norm1.txt',usecols=[5]) + 1j*np.loadtxt('norm1.txt',usecols=[6] )  
	# norm2 = np.loadtxt('norm2.txt',usecols=[5]) + 1j*np.loadtxt('norm2.txt',usecols=[6] )
	# norm3 = np.loadtxt('norm3.txt',usecols=[5]) + 1j*np.loadtxt('norm3.txt',usecols=[6] ) 
	# den2  = poly.polymul((0., 0., -1.),den)
	# dnum2 = poly.polyder(num2) 
	# dden2 = poly.polyder(den2) 
	# dden  = poly.polyder(den) 
	# func1 = 1j*getfunction(eigs[range(n_cum[x],n_cum[x+1])],dden2)
	# func2 = 1j*getfunction(eigs[range(n_cum[x],n_cum[x+1])],dnum2)
	# func3 = 1j*getfunction(eigs[range(n_cum[x],n_cum[x+1])],dden)
	# d1 = 1/cel**2 * func1* norm1
	# d2 = 1/cel**2 * func2* norm2 
	# d3 = -func3* norm3 
	# dd[range(n_cum[x],n_cum[x+1])] = d1+d2+d3
...
for i,omega0 in enumerate(omegas) : 
	...
	for x in range(n_region):
		...
		# Pns = Jn/(omega0-reso)/dd[range(n_cum[x],n_cum[x+1])]*(reso/omega0)**rho*getfunction(omega0,den)
	...
	projection_coeff_getdp = str_getdp_path+'getdp projection_coeff_closed.pro -pre Projection -msh  '+mesh_filename+' -cal -pos postop_modal -v 0' 
